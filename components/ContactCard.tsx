...
export function ContactCard({ icon, title, description, href, label, index }: ContactCardProps) {
  ...
  return (
    <a
      ...
      className={`group p-4 sm:p-6 md:p-8 rounded-xl border border-border hover:border-primary/50 bg-card/50 hover:bg-card transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 text-center hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background ${isVisible ? 'scroll-animate-delay-' + ((index % 5) + 1) : ''}`}
      aria-label={title}
    >
